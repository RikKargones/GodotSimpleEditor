...
func find_children_by_name(seek_name : String, recursive : bool = false, stack : int = 0) -> Array[XMLNode]:
	var nodes : Array[XMLNode]
	
	for node in children:
		if node.name == seek_name:
			nodes.append(node)
		if recursive && node.children.size() > 0 && stack < 100:
			nodes.append_array(node.find_children_by_name(seek_name, recursive, stack + 1))
	
	return nodes


func seek_first_child_by_name(seek_name : String) -> XMLNode:
	for node in children:
		if seek_name == node.name: return node
	return null
