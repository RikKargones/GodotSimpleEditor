...
static func _parse(xml: PackedByteArray) -> XMLDocument:
	...
	while parser.read() != ERR_FILE_EOF:
		...
		# RikK: Except doc declaration! For saving files...
		if node == null:
			#if queue.size() != 0: continue
			if doc.file_declaration.length() != 0: continue
			var text : String = parser.get_node_name()
			if text.begins_with("?") && text.ends_with("?"):
				doc.file_declaration = text.trim_suffix("?").trim_prefix("?")
			continue
		...
		if len(queue) == 0:
			...
			# same here
				...
				var last : Variant = queue.pop_back()  # get-remove last unclosed node
